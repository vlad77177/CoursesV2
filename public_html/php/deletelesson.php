...
if($user['curator']==1){
    $res=mysqli_query($db,'DELETE FROM lessons WHERE id='.$data['id'].' AND id_course In (SELECT id_course FROM curator_course WHERE id_curator='.$user['id'].')');
}
if($user['teacher']==1){
    $res=mysqli_query($db,'DELETE FROM lessons WHERE id='.$data['id'].' AND id_course In (SELECT id_course FROM teacher_course WHERE id_teacher='.$user['id'].')');
}
